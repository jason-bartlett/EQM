...
class ByNameReferenceListProperty extends abstract_property_1.AbstractProperty {
    constructor(declaredOn, parent, name, value, _targetType) {
        ...
        this._viewListObserverDisposer = this._viewList.observe((change) => {
            ...
            switch (change.type) {
                case "splice":
                    const newQualifiedNames = change.added
                        // tslint:disable-next-line:no-unnecessary-type-assertion
                        .map(newValue => newValue._getQualifiedName());
        ...
        const getElementsFromCache = () => this.observableValue.map(qualifiedName => this.parent._model._resolveName(this._targetType, qualifiedName));
        const updateViewList = (newElements) => {
            const oldElements = this._viewList.slice();
            if (utils_1.utils.shallowEquals(oldElements, newElements)) {
                return;
            }
            this._suppressViewEvents = true;
            this._viewList.replace(newElements);
            this._suppressViewEvents = false;
        };
        this._viewListUpdaterDisposer = mobx_1.reaction(getElementsFromCache, updateViewList, { fireImmediately: true });
    }
    /** @internal */
    initialize(value) {
